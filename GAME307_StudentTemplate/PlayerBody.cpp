...
void PlayerBody::Render( float scale )
{
    ...
    float width = image->w * scale;
    float height = image->h * scale;

    // The square's x and y values represent the top left corner of
    // where SDL will draw the .png image
    // The 0.5f * w/h offset is to place the .png so that pos represents the center
    // (Note the y axis for screen coords points downward, hence subtractions!!!!)
    square.x = static_cast<int>(screenCoords.x - 0.5f * width);
    square.y = static_cast<int>(screenCoords.y - 0.5f * height);
    square.w = static_cast<int>(width);
    square.h = static_cast<int>(height);
